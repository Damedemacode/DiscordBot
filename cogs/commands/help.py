...
class Help(commands.Cog):
    ...
    async def help(self, interaction: discord.Integration):
        embedVar = discord.Embed(title="Регистрация: /reg", description="", color=config.bot.embedColor)
        #embedVar.add_field(name=f"{config.bot.prefix}Reg", value=f"Доступна лишь регистрация: {config.bot.prefix}reg", inline=False)
